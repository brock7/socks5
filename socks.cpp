...
{
	DWORD dwThreadID;
	HANDLE ThreadHandle = CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)thread_proc, (LPVOID)p, 0, &dwThreadID);
	return ThreadHandle;
}

void DestroyThread( ThreadDesc thread)
{
	CloseHandle((HANDLE) thread);
}

void WaitThread(ThreadDesc thread)
{
...
typedef pthread_t* ThreadDesc;
ThreadDesc SpawnThread( ThreadProc thread_proc, void* p )
{
	ThreadDesc thread = (ThreadDesc )malloc(sizeof(ThreadDesc));

	if (pthread_create(thread, NULL, thread_proc, p) == 0) {
		return thread;
	}
...
BOOL SendRequest(SOCKET* CSsocket, char *SenderBuf, char *ReceiveBuf, int DataLen)
{
	...
	if(CSsocket[0] && CSsocket[1])
	{
		...
		//printf("%s %s\n",ReqInfo1,ReqInfo2);
		ThreadDesc ThreadHandle = SpawnThread((ThreadProc )TCPTransfer ,(LPVOID)CSsocket);
		if (ThreadHandle != NULL)
		{
			WaitThread(ThreadHandle);
...
char *DNS(char *HostName)
{
	...
	iaddr = *((struct in_addr *)*hostent->h_addr_list);
...
BOOL ConnectToRemoteHost(SOCKET *ServerSocket,char *HostName,const WORD RemotePort)
{
	...
	*ServerSocket = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	...
	if (connect(*ServerSocket, (const struct sockaddr *)&Server,sizeof(Server)) == SOCKET_ERROR)
	{
...
BOOL CreateUDPSocket(Socks5AnsConn *SAC, SOCKET *socks)
{
	...
	socklen_t structsize=sizeof(sockaddr_in);
	...
	if(bind(Locals,(struct sockaddr*)&UDPServer, sizeof(UDPServer)) == SOCKET_ERROR)
	{
	...
	*socks = Locals;
	...
	SAC->_IPandPort.dwIP = inet_addr(GetInetIP(szIP));
	SAC->_IPandPort.wPort = in.sin_port;
...
DWORD WINAPI ZXProxyThread(SOCKET* CSsocket)
{
	...
	{ //ProcessingUDPsession;
		...
		if(inet_addr(HostName)==0)
			sPara.Client._IPandPort.dwIP = in.sin_addr.s_addr;
		else
			sPara.Client._IPandPort.dwIP = inet_addr(DNS(HostName));
		////printf("Accept ip:%s\n",inet_ntoa(in.sin_addr));
		sPara.Client._IPandPort.wPort= htons(RemotePort);/////////////////
		...
		if(SAC.REP==0x01) // general SOCKS server failure
			goto exit;
		sPara.Local._IPandPort=SAC._IPandPort; //Copy local UDPsocket data structure to sPara.Local
		////// Create UDP Transfer thread
		ThreadDesc ThreadHandle = SpawnThread((ThreadProc )UDPTransfer,(LPVOID)&sPara);
		if (ThreadHandle != NULL)
		{
			//printf("Socks%d UDP Session-> %s:%d\n",ProtocolVer,inet_ntoa(in.sin_addr),ntohs(sPara.Client._IPandPort.wPort));
			WaitThread(ThreadHandle);
			DestroyThread(ThreadHandle);
...
BOOL StartProxy()
{
	...
	if(bind(ProxyServer, (struct sockaddr* )&Server, sizeof(Server)) == SOCKET_ERROR)
		return false;
	...
	while(1)
	{
		...
		ThreadDesc hThread = SpawnThread ((ThreadProc)ZXProxyThread, CSsocket);        // Create Thread To Handle Request
...
void WINAPI UDPTransfer(Socks5Para *sPara)////////////////!!!!!!!!!!!!!!!!
{
	...
	UDPClient.sin_addr.s_addr = sPara->Client._IPandPort.dwIP;
	UDPClient.sin_port = sPara->Client._IPandPort.wPort;
	/*/test
	  Socks5UDPHead test;
	  memset(&test,0,sizeof(Socks5UDPHead));
	  test.RSV[0]=0x05;
	  test.ATYP=0x01;
	  test._IPandPort=sPara->Local._IPandPort;
	...
	while(1)
	{
		...
		if(FD_ISSET(sPara->Local.socks, &readfd))
		{
			...
			DataLength=recvfrom(sPara->Local.socks,
					RecvBuf+10, MAXBUFSIZE-10, 0, (struct sockaddr *)&SenderAddr, &SenderAddrSize);
			...
			{//SenderAddr.sin_addr.s_addr==UDPServer.sin_addr.s_addr&&
				...
				memset(UDPHead,0,10);
				UDPHead->ATYP=0x01;
				UDPHead->_IPandPort=sPara->Client._IPandPort;
				//UDPHead->_IPandPort.dwIP =SenderAddr.sin_addr.s_addr;
				//UDPHead->_IPandPort.wPort=SenderAddr.sin_port;
